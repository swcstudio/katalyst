...
> Source: `src/blocks/testimonials-masonry-grid.tsx`
...
<!-- TODO: Add detailed documentation for TestimonialsMasonryGrid -->
...
export function TestimonialsMasonryGrid() {
  ...
  return (
    <div className="py-20">
      ...
      <p className={cn('mt-2 text-lg text-neutral-600 dark:text-neutral-200 sm:text-center')}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 max-w-7xl mx-auto items-start px-4 md:px-8 mt-10">
        {grid.map((testimonialsCol, index) => (
          <div key={`testimonials-col-${index}`} className="grid gap-4 items-start">
...
export const Quote = ({
  ...
  return (
    <h3 className={cn('text-base font-normal dark:text-white text-black py-2 relative', className)}>
      {children}
    </h3>
  );
};
