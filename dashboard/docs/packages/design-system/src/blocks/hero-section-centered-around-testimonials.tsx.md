...
> Source: `src/blocks/hero-section-centered-around-testimonials.tsx`
...
<!-- TODO: Add detailed documentation for CenteredAroundTestimonials -->
...
export function CenteredAroundTestimonials() {
  ...
  useMotionValueEvent(scrollYProgress, 'change', (latest) => {
    setMovedOut(Number.parseFloat(latest.toFixed(2)) > 0.04);
  });
  ...
  const TestimonialCard = ({
    ...
    return (
      <motion.div
        ...
        animate={{
          y: movedOut ? 300 : 0,
          x: movedOut ? (direction === 'left' ? -400 : direction === 'right' ? 400 : 0) : 0,
  ...
  return (
    <div ref={containerRef} className="relative w-full overflow-hidden">
      <TestimonialCard testimonial={testimonials[0]} className="-left-10 top-20" direction="left" />
      <TestimonialCard
