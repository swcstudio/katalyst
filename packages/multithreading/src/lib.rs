...
impl BatchProcessor {
    ...
    pub fn new(config: Option<RayonConfig>) -> Self {
        BatchProcessor {
            pool: RayonThreadPool::new(config),
        }
    }

    #[napi]
    pub fn batch_map(&self, data: Vec<f64>, batch_size: u32, operation: String) -> Vec<f64> {
        let chunks: Vec<_> = data.chunks(batch_size as usize).map(|chunk| {
            chunk.iter().map(|&x| match operation.as_str() {
                "square" => x * x,
                "double" => x * 2.0,
                _ => x,
            }).collect()
        ...
        let results: Vec<f64> = chunks.into_par_iter().flatten().collect();
        results
    }

    #[napi]
    pub fn batch_reduce(&self, data: Vec<f64>, batch_size: u32, operation: String) -> f64 {
...
#[napi]
pub fn create_web_worker_script(script: String) -> String {
    format!(
        "importScripts('data:application/javascript;base64,{encoded}');",
        encoded = base64::encode(script)
    )
}
