  ...
  return (
    <div class={css({ padding: '4', borderRadius: 'md', bg: 'gray.50', _dark: { bg: 'gray.800' } })}>
      ...
      <Show when={posts.error}>
        <div class={css({ p: '4', color: 'red.500' })}>
      ...
      <Show when={!posts.loading && !posts.error}>
        <div class={flex({ direction: 'column', gap: '3' })}>
          <For each={posts()?.slice(0, 5)}>
            {(post) => (
              <div class={css({ p: '3', borderRadius: 'md', bg: 'white', _dark: { bg: 'gray.700' }, boxShadow: 'sm' })}>
                <h3 class={css({ fontSize: 'lg', fontWeight: 'semibold', mb: '2' })}>{post.title}</h3>
                <p class={css({ fontSize: 'sm', color: 'gray.600', _dark: { color: 'gray.300' } })}>{post.body}</p>
              </div>
            )}
          </For>
        </div>
