      ...
      startTransition(async () => {
        ...
        try {
          ...
          switch (actionName) {
            ...
            case 'data_transform':
              output = await nativeModule.rayonParallelFilter(
                input,
                'transform'
              );
              break;
            ...
            case 'background_task':
              output = await nativeModule.tokioSpawnBlocking(
                'background',
                input
              );
              break;
...
export function useParallelServerAction<TInput extends any[], TOutput>(
  ...
  const executeParallel = useCallback(async (inputs: TInput): Promise<TOutput[]> => {
    if (!isInitialized || !nativeModule) {
      throw new Error('Multithreading not initialized');
    }
    ...
    return new Promise((resolve, reject) => {
      startTransition(async () => {
        ...
        try {
          ...
          const errorMessage = error instanceof Error ? error.message : 'Parallel server action failed';
