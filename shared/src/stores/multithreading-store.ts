...
export const useMultithreadingStore = create<MultithreadingStore>()(
  subscribeWithSelector((set, get) => ({
    ...
    addTask: (task: ThreadTask) => set((state) => {
      const newTasks = new Map(state.tasks);
      newTasks.set(task.id, task);
      return { tasks: newTasks };
    }),
    
    updateTask: (id: string, updates: Partial<ThreadTask>) => set((state) => {
      const newTasks = new Map(state.tasks);
    ...
    addThreadPool: (pool: ThreadPool) => set((state) => {
      const newPools = new Map(state.threadPools);
      newPools.set(pool.id, pool);
      return { threadPools: newPools };
    }),
    
    updateThreadPool: (id: string, updates: Partial<ThreadPool>) => set((state) => {
      const newPools = new Map(state.threadPools);
    ...
    removeThreadPool: (id: string) => set((state) => {
      const newPools = new Map(state.threadPools);
      newPools.delete(id);
      return { threadPools: newPools };
    }),
    ...
    updateMetrics: (metrics: Partial<MultithreadingMetrics>) => set((state) => ({
      metrics: {
        ...state.metrics,
        ...metrics,
        lastUpdated: Date.now(),
    ...
    createChannel: (id: string, channel: any) => set((state) => {
      const newChannels = new Map(state.channels);
      newChannels.set(id, channel);
      return { channels: newChannels };
    }),
    
    removeChannel: (id: string) => set((state) => {
      const newChannels = new Map(state.channels);
