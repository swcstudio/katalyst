...
> Source: `src/blocks/stats-for-changelog.tsx`
...
<!-- TODO: Add detailed documentation for StatsForChangelog -->

### `StatsForChangelogMobile`

<!-- TODO: Add detailed documentation for StatsForChangelogMobile -->

### `StatsForChangelogDesktop`

<!-- TODO: Add detailed documentation for StatsForChangelogDesktop -->
      ...
      content: (
        <div className="flex flex-col">
          ...
          <p className="mb-8 mt-4 text-neutral-600 dark:text-neutral-300">
            Minor bug fixes in the template, included new features with beams collision and dark
            mode.
          </p>
          <Image
  ...
  return (
    <div className="mx-auto block w-full max-w-7xl px-4 md:px-8 lg:hidden">
      {stats.map((stat, index) => (
        <div className="mb-10 flex flex-col gap-4" key={`stat-tab-mobile-${index}`}>
          <div
            className={cn(
...
export const StatsForChangelogDesktop = ({ stats }: { stats: Stats[] }) => {
  ...
  const isActive = (stat: (typeof stats)[number]) => stat.title === active.title;
