...
export function validateKatalystConfig(config: KatalystConfig): boolean {
  ...
  if (!Array.isArray(config.features) || !Array.isArray(config.plugins) || !Array.isArray(config.integrations)) {
    return false;
  }

  return config.features.every(validateFeature) &&
...
export function validateIntegration(integration: KatalystIntegration): boolean {
  const validTypes = ['bundler', 'framework', 'ui', 'testing', 'deployment', 'development', 'validation', 'automation'];
