...
Deno.test("MagicUI Components", async (t) => {
  ...
  await t.step("Performance Tests", async (t) => {
    ...
    await t.step("components handle multiple instances", () => {
      ...
      assertEquals(renderTime < 500, true, `Multiple instance render time ${renderTime}ms too high`);
    });
      ...
      components.forEach((component, index) => {
        ...
        if (html.includes('<img')) {
          assertStringIncludes(html, 'alt=', 
            `Component ${index} should have alt text for images`);
        }
      });
    });
    ...
    await t.step("interactive elements are keyboard accessible", () => {
      ...
      interactiveComponents.forEach((component) => {
        ...
        if (html.includes('<button')) {
          ...
          const hasInaccessibleButtons = html.includes('tabindex="-1"') && 
                                        !html.includes('aria-hidden="true"');
          assertEquals(hasInaccessibleButtons, false, 
  ...
  await t.step("Styling and Animation Tests", async (t) => {
    ...
    await t.step("3D components have perspective styling", () => {
      ...
      threeDComponents.forEach((component, index) => {
        const html = renderToString(component);
        const hasPerspective = html.includes('perspective') || 
                              html.includes('transform') ||
                              html.includes('translateZ');
        assertEquals(hasPerspective, true, 
          `3D component ${index} should have 3D styling`);
      });
    });
    ...
    await t.step("components have proper transition animations", () => {
      ...
      animatedComponents.forEach((component, index) => {
        const html = renderToString(component);
        const hasAnimation = html.includes('transition') || 
                            html.includes('animation') ||
                            html.includes('@keyframes');
        assertEquals(hasAnimation, true, 
    ...
    await t.step("components maintain responsive design", () => {
      ...
      components.forEach((component, index) => {
        const html = renderToString(component);
        const isResponsive = html.includes('width') || 
                            html.includes('max-width') ||
                            html.includes('flex');
        assertEquals(isResponsive, true, 
          `Component ${index} should have responsive styling`);
      });
    });
...
Deno.test("MagicUI Components Integration", async (t) => {
  ...
  await t.step("components maintain performance when combined", () => {
    ...
    assertEquals(renderTime < 400, true, 
