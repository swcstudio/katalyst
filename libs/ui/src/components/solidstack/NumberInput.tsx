...
export function NumberInput(props: NumberInputProps) {
  ...
  const [state, send] = useMachine(
    numberInput.machine({
      ...
      onValueChange: (details) => {
        const numValue = Number.parseFloat(details.value);
  ...
  const controlClasses = () => {
    ...
    const variantClasses = {
      ...
      filled:
        'bg-gray-100 border border-transparent rounded-md focus-within:bg-white focus-within:border-blue-500 focus-within:ring-1 focus-within:ring-blue-500',
    };
    ...
    <div {...others} class={containerClasses()}>
      ...
      <div {...api().rootProps}>
        <div {...api().controlProps} class={controlClasses()}>
          <button {...api().decrementTriggerProps} class={buttonClasses()} type="button">
            <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width={2} d="M20 12H4" />
            </svg>
          </button>

          <input {...api().inputProps} class={inputClasses()} />

          <button {...api().incrementTriggerProps} class={buttonClasses()} type="button">
            <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path
