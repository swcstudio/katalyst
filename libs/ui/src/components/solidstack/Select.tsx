...
import { createMemo, createUniqueId, For, JSX, splitProps } from "solid-js"
...
export function Select(props: SelectProps) {
  const [local, others] = splitProps(props, [
    "options", "value", "defaultValue", "placeholder", "disabled", "invalid", 
    "required", "name", "id", "class", "label", "helperText", "errorText",
    "size", "variant", "onChange", "onValueChange"
  ])
  ...
  const baseClasses = "relative w-full transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50"
          ...
          {api().open && (
            <div {...api().getContentProps()} class={contentClasses()}>
              <For each={local.options}>
                {(option) => (
                  <div
                    {...api().getOptionProps({ label: option.label, value: option.value, disabled: option.disabled })}
