...
import { createUniqueId, createSignal, createMemo } from 'solid-js'
...
export function createTestimonialSectionMachine(options: TestimonialSectionMachineOptions = {}) {
  ...
  const send = (event: TestimonialSectionEvent) => {
    ...
    switch (event.type) {
      ...
      case 'TESTIMONIAL_COMPLETED':
        if ('testimonialId' in event) {
          setContext(prev => {
            ...
            const allCompleted = newCompletedTestimonials.size === prev.testimonialData.testimonials.length
            return { 
              ...prev, 
          ...
          if (context().completedTestimonials.size === context().testimonialData.testimonials.length) {
            setState('complete')
      ...
      case 'NEXT_SLIDE':
        setContext(prev => {
          const maxSlide = Math.ceil(prev.testimonialData.testimonials.length / prev.slidesPerView) - 1
...
export function useTestimonialSection(options: TestimonialSectionMachineOptions = {}): TestimonialSectionAPI {
  ...
  const filteredTestimonials = createMemo(() => {
    ...
    return testimonials.filter(testimonial => testimonial.category === state.context.filterCategory)
  })
  ...
  const totalSlides = createMemo(() => {
    return Math.ceil(state.context.testimonialData.testimonials.length / state.context.slidesPerView)
  })
  
  return {
    // State queries
    get isInitializing() { return state.value === 'initializing' },
    get isIdle() { return state.value === 'idle' },
    get isLoading() { return state.value === 'loading' },
    get isAnimating() { return state.value === 'animating' },
    get isInteractive() { return state.value === 'interactive' },
    get isComplete() { return state.value === 'complete' },
    get isError() { return state.value === 'error' },
    
    // Context accessors
    get testimonialData() { return state.context.testimonialData },
    get activeTestimonialId() { return state.context.activeTestimonialId },
    get hoveredTestimonialId() { return state.context.hoveredTestimonialId },
    get visibleTestimonials() { return state.context.visibleTestimonials },
    get animatedTestimonials() { return state.context.animatedTestimonials },
    get completedTestimonials() { return state.context.completedTestimonials },
    get animationPhase() { return state.context.animationPhase },
    get theme() { return state.context.theme },
    get variant() { return state.context.variant },
    get isVisible() { return state.context.isVisible },
    get autoplayEnabled() { return state.context.autoplayEnabled },
    get filterCategory() { return state.context.filterCategory },
    get sortOrder() { return state.context.sortOrder },
    get showRatings() { return state.context.showRatings },
    get currentSlide() { return state.context.currentSlide },
    get slidesPerView() { return state.context.slidesPerView },
    get errorState() { return state.context.errorState },
    ...
    selectTestimonial: (testimonialId: string) => send({ type: 'SELECT_TESTIMONIAL', testimonialId }),
    ...
    updateTestimonials: (testimonials: Testimonial[]) => send({ type: 'UPDATE_TESTIMONIALS', testimonials }),
    ...
    markTestimonialVisible: (testimonialId: string) => send({ type: 'TESTIMONIAL_VISIBLE', testimonialId }),
    markTestimonialAnimated: (testimonialId: string) => send({ type: 'TESTIMONIAL_ANIMATED', testimonialId }),
    markTestimonialCompleted: (testimonialId: string) => send({ type: 'TESTIMONIAL_COMPLETED', testimonialId }),
    retry: () => send({ type: 'RETRY' }),
    
    // Computed properties
    get filteredTestimonials() { return filteredTestimonials() },
    get sortedTestimonials() { return sortedTestimonials() },
    get featuredTestimonials() { return featuredTestimonials() },
    get animationProgress() { return animationProgress() },
    get totalSlides() { return totalSlides() },
    
    isTestimonialVisible: (testimonialId: string) => state.context.visibleTestimonials.has(testimonialId),
    isTestimonialAnimated: (testimonialId: string) => state.context.animatedTestimonials.has(testimonialId),
    isTestimonialCompleted: (testimonialId: string) => state.context.completedTestimonials.has(testimonialId),
    isTestimonialHovered: (testimonialId: string) => state.context.hoveredTestimonialId === testimonialId,
    isTestimonialSelected: (testimonialId: string) => state.context.activeTestimonialId === testimonialId
  }
}