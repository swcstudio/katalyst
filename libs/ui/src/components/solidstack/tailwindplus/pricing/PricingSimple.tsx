import { Component, JSX, onMount, createSignal, For, createEffect } from "solid-js";
...
export const PricingSimple: Component<PricingSimpleProps> = (props) => {
  ...
  const badgeStyles = css({
    ...
    border: theme() === "dark" ? "1px solid rgba(129, 140, 248, 0.2)" : "1px solid rgba(99, 102, 241, 0.2)",
  });
  ...
  const frequencyButtonStyles = (isActive: boolean) => css({
    padding: "8px 16px",
    fontSize: "14px",
    fontWeight: "600",
    borderRadius: "8px",
  ...
  const tiersGridStyles = css({
    ...
    margin: "0 auto",
    "@media (min-width: 640px)": {
      gridTemplateColumns: layout() === "row" ? "1fr" : 
                          props.tiers.length === 2 ? "repeat(2, 1fr)" :
                          props.tiers.length >= 3 ? "repeat(2, 1fr)" : "1fr",
      gap: "40px",
    },
    "@media (min-width: 1024px)": {
      gridTemplateColumns: layout() === "row" ? "1fr" :
                          props.tiers.length === 2 ? "repeat(2, 1fr)" :
                          props.tiers.length >= 3 ? "repeat(3, 1fr)" : "1fr",
      gap: "32px",
    },
  });
  ...
  const tierCardStyles = (tier: PricingTier) => {
    ...
    return css({
      ...
      border: isPopular || isFeatured
        ? (theme() === "dark" ? "2px solid #6366f1" : "2px solid #6366f1")
        : (theme() === "dark" ? "1px solid #374151" : "1px solid #e5e7eb"),
      boxShadow: isPopular || isFeatured
        ? (theme() === "dark" 
  ...
  const ctaButtonStyles = (tier: PricingTier) => {
    ...
    return css({
      ...
      transition: "all 0.2s ease",
      display: "block",
      backgroundColor: isPopular || isFeatured 
        ? "#6366f1"
        : (theme() === "dark" ? "#374151" : "#f3f4f6"),
      color: isPopular || isFeatured 
        ? "#ffffff"
        : (theme() === "dark" ? "#ffffff" : "#111827"),
      border: isPopular || isFeatured 
        ? "none"
        : (theme() === "dark" ? "1px solid #4b5563" : "1px solid #d1d5db"),
      "&:hover": {
        backgroundColor: isPopular || isFeatured 
          ? "#5b5bd6"
          : (theme() === "dark" ? "#4b5563" : "#e5e7eb"),
  ...
  const getPriceSuffix = () => {
    const currentFreq = frequencies().find(f => f.value === pricingSection.getSelectedFrequency());
                ...
                <BlurFade delay={0.7}>
                  <div class={frequencyButtonGroupStyles}>
                    <For each={frequencies()}>
                      {(frequency) => (
                        <button
                          class={frequencyButtonStyles(pricingSection.getSelectedFrequency() === frequency.value)}
