...
export interface SupportSectionAPI {
  ...
  submitContact: (cardId: string, data: Record<string, unknown>) => void;
...
export function useSupportSection(): SupportSectionAPI {
  const [currentState, setCurrentState] = createSignal<
    'initializing' | 'idle' | 'browsing' | 'interacting' | 'contacting' | 'submitting' | 'filtering'
  >('initializing');
  ...
  const submitContact = (cardId: string, data: Record<string, unknown>) => {
    setContext('activeCard', cardId);
  ...
  const getFilteredCards = (): SupportCard[] => {
    ...
    if (context.filterMode === 'available') {
      ...
      filtered = filtered.filter(
        (card) => card.contactMethod === 'phone' || card.contactMethod === 'chat'
      );
    }
