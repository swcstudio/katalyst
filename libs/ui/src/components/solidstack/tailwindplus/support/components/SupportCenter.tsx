...
export const SupportCenter: Component<SupportCenterProps> = (props) => {
  ...
  const filterButtonStyles = (active: boolean) => css({
    paddingX: "16px",
    paddingY: "8px",
    borderRadius: "8px",
    fontSize: "14px",
    fontWeight: "500",
    border: "1px solid",
    borderColor: active ? (theme() === "dark" ? "#6366f1" : "#4f46e5") : (theme() === "dark" ? "rgba(255, 255, 255, 0.1)" : "#e5e7eb"),
    backgroundColor: active ? (theme() === "dark" ? "rgba(99, 102, 241, 0.1)" : "rgba(79, 70, 229, 0.1)") : "transparent",
    color: active ? (theme() === "dark" ? "#a5b4fc" : "#4f46e5") : (theme() === "dark" ? "#d1d5db" : "#6b7280"),
    cursor: "pointer",
    transition: "all 0.2s ease-in-out",
    "&:hover": {
      borderColor: theme() === "dark" ? "#6366f1" : "#4f46e5",
  ...
  const cardStyles = (card: SupportCard) => css({
    position: "relative",
    display: "flex",
    gap: "16px",
  ...
  const cardIconContainerStyles = (card: SupportCard) => css({
    flexShrink: "0",
    display: "flex",
    alignItems: "center",
    ...
    height: "48px",
    borderRadius: "12px",
    backgroundColor: card.priority ? (theme() === "dark" ? "#6366f1" : "#4f46e5") : (theme() === "dark" ? "rgba(129, 140, 248, 0.1)" : "rgba(79, 70, 229, 0.1)"),
  ...
  const contactMethodStyles = (method: string) => css({
    paddingX: "12px",
    paddingY: "6px",
    borderRadius: "6px",
    fontSize: "12px",
  ...
  const availabilityBadgeStyles = (available: boolean) => css({
    display: "inline-flex",
    alignItems: "center",
    gap: "6px",
    paddingX: "8px",
    ...
    fontSize: "12px",
    fontWeight: "500",
    backgroundColor: available ? (theme() === "dark" ? "rgba(34, 197, 94, 0.1)" : "rgba(34, 197, 94, 0.1)") : (theme() === "dark" ? "rgba(239, 68, 68, 0.1)" : "rgba(239, 68, 68, 0.1)"),
    color: available ? (theme() === "dark" ? "#4ade80" : "#16a34a") : (theme() === "dark" ? "#f87171" : "#dc2626"),
  });

  const statusDotStyles = (available: boolean) => css({
    width: "6px",
    height: "6px",
    borderRadius: "50%",
    backgroundColor: available ? (theme() === "dark" ? "#4ade80" : "#16a34a") : (theme() === "dark" ? "#f87171" : "#dc2626"),
  });
  ...
  const overlappingCardStyles = (index: number) => css({
    display: "flex",
    flexDirection: "column",
    borderRadius: "16px",
                ...
                <BlurFade delay={0.5 + index() * 0.1} inView>
                  ...
                  >
                    ...
                    <div class={cardContentStyles}>
                      <div class={css({ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "8px" })}>
  ...
  const renderOverlappingVariant = () => (
    <>
      <div class={css({ position: "relative", backgroundColor: "#1f2937", paddingBottom: "128px" })}>
        <Show when={props.heroImage}>
          <div class={css({ position: "absolute", inset: "0" })}>
            ...
            />
            <div class={css({ position: "absolute", inset: "0", backgroundColor: "#1f2937", mixBlendMode: "multiply" })} />
          </div>
        </Show>

        <div class={css({ position: "relative", marginX: "auto", maxWidth: "1792px", paddingX: "24px", paddingY: "96px", "@media (min-width: 1024px)": { paddingX: "32px" } })}>
          <TextAnimate
            class={css({ fontSize: "32px", fontWeight: "700", letterSpacing: "-0.025em", color: "#ffffff", "@media (min-width: 768px)": { fontSize: "48px" }, "@media (min-width: 1024px)": { fontSize: "64px" } })}
            animation="slideUp"
            delay={0.1}
          >
            {props.title || "Support"}
          ...
          <Show when={props.subtitle}>
            <BlurFade delay={0.2} inView>
              <p class={css({ marginTop: "24px", maxWidth: "768px", fontSize: "20px", color: "#d1d5db" })}>
                {props.subtitle}
              </p>
            </BlurFade>
          </Show>
      ...
      <section class={overlappingCardsStyles}>
        <div class={css({ display: "grid", gridTemplateColumns: "1fr", gap: "80px", "@media (min-width: 1024px)": { gridTemplateColumns: "repeat(3, 1fr)", gap: "32px" } })}>
          <For each={supportSection.getFilteredCards()}>
            {(card, index) => (
              <BlurFade delay={0.3 + index() * 0.1} inView>
                <div class={overlappingCardStyles(index())}>
                  <div class={overlappingCardHeaderStyles}>
                    ...
                    <h3 class={css({ fontSize: "20px", fontWeight: "500", color: "#111827" })}>{card.name}</h3>
                    <p class={css({ marginTop: "16px", fontSize: "16px", color: "#6b7280" })}>{card.description}</p>
                  </div>
                  <div class={css({ borderRadius: "0 0 16px 16px", backgroundColor: "#f9fafb", padding: "24px", "@media (min-width: 768px)": { paddingX: "32px" } })}>
                    <ShimmerButton
                      class={css({ width: "100%" })}
  ...
  return (
    <div class={`${containerStyles} ${props.className || ""}`}>
      ...
      <Show when={variant() === "simple"}>
        {renderSimpleVariant()}
      </Show>

      <Show when={variant() === "hero" || variant() === "cards"}>
        {renderHeroVariant()}
      </Show>

      <Show when={variant() === "split"}>
        {renderOverlappingVariant()}
      </Show>
    </div>
  );
};