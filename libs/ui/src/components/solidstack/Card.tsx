...
export function Card(props: CardProps) {
  const [local, others] = splitProps(props, [
    "variant", "size", "padding", "header", "footer", "children", 
    "headerClass", "bodyClass", "footerClass", "class"
  ])
  ...
  const headerClasses = () => {
    const padding = local.padding === "none" ? "" : local.padding === "sm" ? "px-3 pt-3 pb-2" : local.padding === "lg" ? "px-8 pt-8 pb-4" : "px-6 pt-6 pb-4"
    return `border-b border-gray-200 ${padding} ${local.headerClass || ""}`
  }

  const footerClasses = () => {
    const padding = local.padding === "none" ? "" : local.padding === "sm" ? "px-3 pb-3 pt-2" : local.padding === "lg" ? "px-8 pb-8 pt-4" : "px-6 pb-6 pt-4"
  ...
  const contentPadding = () => {
    ...
    if (local.header && local.footer) {
      return local.padding === "sm" ? "px-3 py-2" : local.padding === "lg" ? "px-8 py-4" : "px-6 py-4"
    } else if (local.header) {
      return local.padding === "sm" ? "px-3 pb-3 pt-2" : local.padding === "lg" ? "px-8 pb-8 pt-4" : "px-6 pb-6 pt-4"
    } else if (local.footer) {
      return local.padding === "sm" ? "px-3 pt-3 pb-2" : local.padding === "lg" ? "px-8 pt-8 pb-4" : "px-6 pt-6 pb-4"
    }
  ...
  return (
    <div {...others} class={cardClasses()}>
      ...
      <div class={`${contentPadding()} ${bodyClasses()}`}>
        {local.children}
      </div>
